<template>
  <div>
    ...
    <div class="search">
      ...
      <button v-if="search.length > 0" class="clear-search-btn" @click="clearSearchBar">
    ...
    <table v-show="!empty && !loading && !failure">
      <thead>
        <tr>
          <th class="table-sort title" :class="currentSortDir" @click="toggleSort()">
            <span>Food site</span>
          </th>
...
const endpoint = "https://services.arcgis.com/fLeGjb7u4uXqeF9q/arcgis/rest/services/Distribution_Sites_PublicViewLayer/FeatureServer/0/query?where=1%3D1&&outFields=*&f=pjson";
...
export default {
  ...
  methods: {
    ...
    searchTable: function() {
      if (this.search) {
        this.$search(this.search, this.mealSites, this.searchOptions).then(
          sites => {
